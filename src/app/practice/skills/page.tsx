...
import { SKILL_TAXONOMY } from "@/lib/taxonomy";
  ...
  return (
    <AppShell>
      <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        <h1 className="mt-3 text-3xl font-semibold tracking-tight text-slate-950">Practice by Reading Skill</h1>
        <p className="mt-3 max-w-3xl leading-7 text-slate-600">
          Choose the reading behaviour you want to improve, review a short strategy, then complete a focused drill.
        </p>
        <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {SKILL_TAXONOMY.map((item) => {
            ...
            return (
              <article key={item.id} className="rounded-md border border-slate-200 bg-white p-5">
                ...
                {drills.length ? (
                  <Link
                    href={`/practice/skills/${item.slug}`}
